...
export interface AudioPacket { pcm: string; sample_rate: number; alignment: Alignment; visemes: VisemeCue[]; mood: Mood; text: string }
export class AudioPlayer {
  ...
  schedule(packet: AudioPacket): Speech {
    ...
    return {clock:this.context,start,duration:buffer.duration,samples,sampleRate:packet.sample_rate,visemes:packet.visemes,alignment:packet.alignment,mood:packet.mood,text:packet.text};
  }
  get pending() { return this.sources.size>0; }
  stop() { for(const s of this.sources){s.stop();s.disconnect();} this.sources.clear();this.next=0; }
